 ...
 */
export function getAppReloadMiddlewareConfig() {
    return {
 ...
 */
export function getPreviewMiddlewareConfig({ ui5Theme, appId, flpAction, localStartFile }) {
 ...
 */
export function getBackendComments(backend, index) {
    const comment = [];
 ...
 */
export function getFioriToolsProxyMiddlewareConfig(backends, ui5, afterMiddleware = 'compression', ignoreCertErrors = false) {
...
export const getMockServerMiddlewareConfig = (basePath, webappPath, dataSourcesConfig, annotationsConfig) => {
    const services = [];
    // Populate services based on dataSourcesConfig
    dataSourcesConfig.forEach((dataSource) => {
        const serviceRoot = `.${posix.sep}${relative(basePath, join(webappPath, 'localService', dataSource.serviceName)).replaceAll(sep, posix.sep)}`;
