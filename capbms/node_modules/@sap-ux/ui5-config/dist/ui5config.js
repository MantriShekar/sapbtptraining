...
import { getAppReloadMiddlewareConfig, getBackendComments, getFioriToolsProxyMiddlewareConfig, getMockServerMiddlewareConfig } from './middlewares.js';
...
export class UI5Config {
    ...
    async validateSchema() {
        if (!UI5Config.validate) {
            ...
            UI5Config.validate = new Ajv({ strict: false }).compile(schema);
        }
        let isValid = false;
        try {
    ...
    addFioriToolsProxyMiddleware(proxyConfig, afterMiddleware) {
        ...
        const { config, comments } = getFioriToolsProxyMiddlewareConfig(proxyConfig.backend, proxyConfig.ui5, afterMiddleware, resolvedIgnoreCertErrors);
        this.document.appendTo({
            path: 'server.customMiddleware',
            value: config,
            comments: comments
    ...
    getFioriToolsProxyMiddlewareConfiguration() {
        ...
        const proxyMiddleware = this.document.findItem(middlewareList, (item) => item.name === fioriToolsProxy);
        if (!proxyMiddleware) {
    ...
    removeBackendFromFioriToolsProxyMiddleware(path) {
        const fioriToolsProxyMiddleware = this.findCustomMiddleware(fioriToolsProxy);
    ...
    addServiceToMockserverMiddleware(basePath, webappPath, dataSourceConfig, annotationsConfig = []) {
        ...
        if (!mockserverMiddleware) {
            ...
            const serviceRoot = `.${posix.sep}${relative(basePath, join(webappPath, 'localService', dataSourceConfig.serviceName)).replaceAll(sep, posix.sep)}`;
    ...
    addAbapDeployTask(target, app, fioriTools = true, exclude, index = false, lrep, comments = []) {
        ...
        const configuration = { target, app, exclude: deployExclude, lrep };
        if (index) {
    ...
    addServeStaticConfig(serveStaticPaths) {
        const serveStaticConfig = this.findCustomMiddleware(serveStatic);
