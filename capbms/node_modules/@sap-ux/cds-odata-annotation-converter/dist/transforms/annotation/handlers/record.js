...
import { COMMON_CASE_ISSUE, DEPRECATED_$VALUE_SYNTAX, Range, TextEdit, createElementNode, createAttributeNode, Edm, DiagnosticSeverity, MISSING_REQUIRED_PROPERTY } from '@sap-ux/odata-annotation-core-types';
...
export const recordHandler = {
    ...
    convert(state, node, parent) {
        ...
        if (explicitType !== undefined) {
            element.attributes[Edm.Type] = createAttributeNode(Edm.Type, explicitType, typeProperty ? nodeRange(typeProperty.name, true) : undefined, typeProperty?.value?.type === STRING_LITERAL_TYPE ? nodeRange(typeProperty.value, false) : undefined);
        }
        return element;
    }
};
...
function getChildren(state, node) {
    ...
    const recordProperties = node.properties.filter((property) => property.name.value !== ReservedProperties.Type &&
...
function addDiagnosticForExtraneousProperties(state, node, valueProperty) {
    ...
    if (valueProperty?.name?.value === ReservedProperties.Value ||
