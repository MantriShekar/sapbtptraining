...
import { RECORD_TYPE, nodeRange, STRING_LITERAL_TYPE, COLLECTION_TYPE, EMPTY_VALUE_TYPE, BOOLEAN_TYPE, NUMBER_LITERAL_TYPE } from '@sap-ux/cds-annotation-parser';
...
export class EdmJsonVisitor {
    ...
    convertAttribute(property, element) {
        if (property.value) {
            ...
            element.attributes[name] = createAttributeNode(name, property.value?.type === STRING_LITERAL_TYPE ? property.value.value : '', nodeRange(property.name, false), property.value.range);
    ...
    convertElementName(property, element, parent) {
        ...
        if (property.value?.type === STRING_LITERAL_TYPE) {
            const textNode = createTextNode(property.value.value, nodeRange(property.value, false));
    ...
    [STRING_LITERAL_TYPE] = (node) => {
        const textNode = createTextNode(node.value, nodeRange(node, false));
    ...
    [NUMBER_LITERAL_TYPE] = (node, parent) => {
