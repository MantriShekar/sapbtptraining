...
export const toEnumValue = (type, memberName) => [type, memberName].join(ENUM_MEMBER_DELIMITER);
export const enumHandler = {
    type: ENUM_TYPE,
    convert(state, node) {
 ...
 */
export function convertFlags(state, node, valueType) {
    // Alternative is to provide conversion diagnostics
    const enums = node.items.filter((item) => item.type === ENUM_TYPE);
    const segments = [];
