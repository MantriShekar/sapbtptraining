import { getAstNodes, ANNOTATION_TYPE, COLLECTION_TYPE, NUMBER_LITERAL_TYPE, RECORD_PROPERTY_TYPE, RECORD_TYPE, ReservedProperties, EMPTY_VALUE_TYPE, ENUM_TYPE, nodeRange, PATH_TYPE, STRING_LITERAL_TYPE, findAnnotationNode } from '@sap-ux/cds-annotation-parser';
import { isBefore, ATTRIBUTE_TYPE, Edm, ELEMENT_TYPE, positionContained, TEXT_TYPE } from '@sap-ux/odata-annotation-core';
/**
 ...
 */
export function findNode(assignment, annotations, position) {
...
function edmJsonContent(astNodes, value, position) {
    const edmJsonIndex = astNodes.findIndex((node) => !Array.isArray(node) && node.type === RECORD_PROPERTY_TYPE && node.name.value === ReservedProperties.EdmJson);
    if (edmJsonIndex === -1) {
        return value;
    }
