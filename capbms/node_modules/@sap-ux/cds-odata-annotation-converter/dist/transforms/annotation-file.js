...
import { getElementAttribute, getElementAttributeValue, REFERENCE_TYPE, TARGET_TYPE, Edm, ELEMENT_TYPE, positionContained, isBefore, elementsWithName, getSingleTextNode } from '@sap-ux/odata-annotation-core';
...
export const adjustCdsTermNames = (assignment, cdsVocabulary) => {
...
export const toAssignment = (annotation, vocabularyService) => {
    ...
    adjustCdsTermNames(assignment, vocabularyService.cdsVocabulary);
    return assignment;
};
export const toTarget = (carrier, carrierName, compilerFacade) => {
...
export const toTargetMap = (fileIndex, uri, vocabularyService, cdsCompilerFacade) => {
...
export const toAnnotationFile = (fileUri, vocabularyService, cdsAnnotationFile, metadataCollector, position, propagationMap, mergePropagatedAnnotations = false) => {
...
const getRecordPropertyElement = (recordElement, propertyName) => {
    return (recordElement?.content ?? []).find((child) => child.type === ELEMENT_TYPE &&
...
function addMdPathsFromAnnotations(terms, mdPathSet) {
    terms
        .filter((t) => ['Common.ValueList', 'com.sap.vocabularies.Common.v1.ValueList'].includes(getElementAttributeValue(t, Edm.Term)))
        .forEach((term) => {
