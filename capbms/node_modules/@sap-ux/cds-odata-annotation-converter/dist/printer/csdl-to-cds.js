import { Edm, ELEMENT_TYPE, TEXT_TYPE, printOptions as defaultPrintOptions, getElementAttributeValue } from '@sap-ux/odata-annotation-core';
import { valuePair, struct, collection, stringLiteral, delimitedIdentifier, keyAlone, PRIMITIVE_VALUE_ATTRIBUTE_NAMES, CDS_NULL_EXPRESSION_LITERAL } from './primitives.js';
 ...
 */
export var PrintPattern;
(function (PrintPattern) {
    PrintPattern["artifact"] = "artifact";
    PrintPattern["element"] = "element";
    PrintPattern["parameter"] = "parameter";
    PrintPattern["boundAction"] = "boundAction";
    PrintPattern["boundParameter"] = "boundParameter"; // annotate <entity> with actions { <action> ( <parameter> <annotation> ) }
 ...
 */
export const resolveTarget = (targetPath) => {
    let printPattern;
    let rootElementName = '';
 ...
 */
function formatTargetResult(target, result, rootElementName, boundActionFunctionName, childSegments, hasMultipleTerms, complexTypePathSegments) {
    ...
    if (childSegments.length === 1) {
        if (boundActionFunctionName) {
            return getTargetForBoundActionFunctions(target, result, rootElementName, boundActionFunctionName, childSegments[0]);
        }
...
export const printTarget = (target, complexTypePathSegments) => {
    ...
    result = formatTargetResult(target, result, rootElementName, boundActionFunctionName, childSegments, terms.length > 1, complexTypePathSegments);
...
export const print = (node, formatterOptions = defaultPrintOptions, printOptions = {}) => {
    const { indentResult, annotationContext } = getPrintOptions(printOptions);
    if (Array.isArray(node)) {
        const nodes = node;
        const withTrailingCommas = nodes
            .map((item) => printCsdlNode(item, annotationContext, formatterOptions) + ',')
            .join('\n');
...
export const printCsdlNode = (node, context, options) => {
    switch (node.type) {
        case ELEMENT_TYPE:
    ...
    switch (node.name) {
        ...
        default: {
            if (node.content && node.content.length > 0) {
                return node.content
                    .map((nodeContent) => nodeContent.type === TEXT_TYPE
...
const annotationFilter = (node) => node.type === ELEMENT_TYPE && node.name === Edm.Annotation;
const encodeSnippet = (options, text) => options.useSnippetSyntax ? text.replace(/\\/g, '\\\\').replace(/\$/g, '\\$') : text;
const printValue = (element, context, options) => {
    const valueElement = (element.content ?? []).find((node) => node.type === ELEMENT_TYPE && node.name !== Edm.Annotation) ??
        (element.content ?? []).find((node) => node.type !== ELEMENT_TYPE && node.text.trim() !== '');
    const edmDynamicTypes = [Edm.If, Edm.Not, Edm.Apply];
...
const printRecord = (element, context, options) => {
    ...
    const skipTextNodes = content.find((child) => child.type === ELEMENT_TYPE && child.name !== Edm.Annotation) !== undefined;
    const properties = content
        .filter((child) => (!skipTextNodes && child.type !== ELEMENT_TYPE) ||
...
export const printPrimitiveValue = (expressionName, expressionValue) => {
    let result = '';
    let enumValues;
