 ...
 */
export function yamlDocumentToYamlString(yamlDocument) {
...
export class YamlDocument {
    ...
    constructor(serializedYaml) {
        ...
        if (docsWithErrors.length > 0) {
            throw new YAMLError(errorTemplate.yamlParsing +
                '\n' +
                docsWithErrors.map((doc) => doc.errors.map((e) => e.message).join('')), errorCode.yamlParsing);
        }
    }
    /**
    ...
    setIn({ path, value, createIntermediateKeys, comment }) {
        ...
        if (pathArray.length > 1) {
            ...
            if (!parentNode && !createIntermediateKeys) {
                ...
                throw new YAMLError(interpolate(errorTemplate.pathDoesNotExist, { path: parentPath }), errorCode.pathDoesNotExist);
            }
        }
        const newNode = this.documents[0].createNode(value);
        if (comment) {
    ...
    appendTo({ path, value, createIntermediateKeys = true, nodeComment, comments }) {
        ...
        if (!seq) {
            ...
            throw new YAMLError(interpolate(errorTemplate.tryingToAppendToNonSequence, { path }), errorCode.tryingToAppendToNonSequence);
        }
        ...
        if (comments && comments.length > 0) {
            if (typeof value !== 'object') {
                throw new YAMLError(errorTemplate.scalarValuesDoNotHaveProperties, errorCode.scalarValuesDoNotHaveProperties);
            }
            ...
            for (const c of comments) {
                ...
                if (!n) {
                    throw new YAMLError(interpolate(errorTemplate.propertyNotFound, { path: c.path }), errorCode.propertyNotFound);
                }
                n.comment = c.comment;
            }
        }
    ...
    updateAt({ path, matcher, value, mode = 'merge' }) {
        ...
        if (!node) {
            throw new YAMLError(interpolate(errorTemplate.nodeNotFoundMatching, { path, key: matcher.key, value: matcher.value }), errorCode.nodeNotFoundMatching);
        }
