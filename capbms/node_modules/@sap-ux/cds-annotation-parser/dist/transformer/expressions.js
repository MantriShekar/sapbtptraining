...
import { NUMBER_LITERAL_TYPE, OPERATOR_TYPE, CORRECT_EXPRESSION_TYPE, INCORRECT_EXPRESSION_TYPE, EXPRESSION_TYPES } from './annotation-ast-nodes.js';
 ...
 */
export function getEdmOperatorMap() {
 ...
 */
export function buildExpression(protoExpression) {
...
function filterNodes(nodes, range) {
    return (nodes || []).filter((node) => node.range && isBefore(range.start, node.range.start, true) && isBefore(node.range.end, range.end, true));
}
/**
 ...
 */
export function rebuildNumberSigns(protoExpression) {
...
function getSortedNodes(protoExpression) {
    return [...protoExpression.operators, ...protoExpression.operands]
        ...
        .sort((node1, node2) => node1.range && node2.range && isBefore(node1.range.start, node2.range.start) ? -1 : +1);
}
/**
 ...
 */
export function positionIsInExpressionWhiteSpace(expression, position) {
 ...
 */
export function containsIncorrectExpressions(expression) {
    ...
    for (const operand of expression.operands) {
        if (EXPRESSION_TYPES.includes(operand.type) && containsIncorrectExpressions(operand)) {
            return true;
        }
    }
    return false;
