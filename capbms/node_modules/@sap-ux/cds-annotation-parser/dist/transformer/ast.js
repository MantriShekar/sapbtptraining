...
import { ANNOTATION_TYPE, ANNOTATION_GROUP_TYPE, ANNOTATION_GROUP_ITEMS_TYPE, ENUM_TYPE, RECORD_TYPE, RECORD_PROPERTY_TYPE, COLLECTION_TYPE, BOOLEAN_TYPE, PATH_TYPE, STRING_LITERAL_TYPE, IDENTIFIER_TYPE, QUALIFIER_TYPE, EMPTY_VALUE_TYPE, NUMBER_LITERAL_TYPE, TOKEN_TYPE, QUOTED_LITERAL_TYPE, SEPARATOR_TYPE, MULTI_LINE_STRING_LITERAL_TYPE, OPERATOR_TYPE, Delimiter, UNSUPPORTED_OPERATOR_EXPRESSION_TYPE, INCORRECT_EXPRESSION_TYPE } from './annotation-ast-nodes.js';
...
const adjustPosition = (position, offset) => Position.create(position.line + offset.line, position.line === 0 ? position.character + offset.character : position.character);
export const locationToRange = (location) => {
    return location
        ? Range.create(location.startLine - 1, (location.startColumn ?? 0) - 1, (location.endLine ?? 0) - 1, location.endColumn ?? 0)
        : undefined;
};
const existsAndNotRecovered = (nodes) => hasItems(nodes) && !nodes[0]?.isInsertedInRecovery;
...
class CstToAstVisitor extends Visitor {
    ...
    locationToRange(location) {
        ...
        if (this.startPosition && range) {
            return Range.create(adjustPosition(range.start, this.startPosition), adjustPosition(range.end, this.startPosition));
        }
        return range;
    }
    /**
    ...
    toTopLevelAnnotation(assignment, location) {
        if (isDefined(assignment.location) &&
    ...
    expression(context, location) {
        ...
        try {
            ...
            return { ...expression, type: INCORRECT_EXPRESSION_TYPE, message: e.toString() };
        }
    }
    /**
     ...
     */
    tokenToIdentifier(token, delimiter = Delimiter.none) {
        return {
            type: IDENTIFIER_TYPE,
            value: token.image,
            range: this.tokenToRange(token),
     ...
     */
    tokenToSeparator(token, delimiter = Delimiter.none) {
        return {
            type: SEPARATOR_TYPE,
            value: token.image,
            range: this.tokenToRange(token),
    ...
    struct(context, location) {
        ...
        const { properties: allProperties, annotations: allAnnotations } = (context.assignment ?? [])
            ...
            if (hasItems(assignment.children.Colon) &&
...
export const buildAst = (cst, tokenVector, startPosition) => {
