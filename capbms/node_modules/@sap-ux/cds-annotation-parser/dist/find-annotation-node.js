import { ANNOTATION_GROUP_TYPE, COLLECTION_TYPE, RECORD_TYPE, ANNOTATION_TYPE, ANNOTATION_GROUP_ITEMS_TYPE, IDENTIFIER_TYPE, ENUM_TYPE, BOOLEAN_TYPE, STRING_LITERAL_TYPE, NUMBER_LITERAL_TYPE, PATH_TYPE, RECORD_PROPERTY_TYPE, nodeRange, EMPTY_VALUE_TYPE, CORRECT_EXPRESSION_TYPE, UNSUPPORTED_OPERATOR_EXPRESSION_TYPE, INCORRECT_EXPRESSION_TYPE, OPERATOR_TYPE } from './transformer/index.js';
 ...
 */
export const findAnnotationNode = (assignment, options) => assignment ? visitor.visit(assignment, options).join('/') : '';
/**
 ...
 */
export function getAstNodes(root, path) {
    const segments = path.split('/');
    let node = root;
    const nodes = [];
    for (let i = 1; i < segments.length; i++) {
