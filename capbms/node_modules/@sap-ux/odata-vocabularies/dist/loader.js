...
import { PROPERTY_KIND, TYPE_DEFINITION_KIND, ENUM_TYPE_KIND, COMPLEX_TYPE_KIND, TERM_KIND } from '@sap-ux/odata-annotation-core-types';
...
function parseTypeDefinition(name, raw) {
    const typeDef = { kind: TYPE_DEFINITION_KIND, name: name };
 ...
 * @returns Vocabulary Object
 */
export function parseEnumTypeDefinition(name, raw) {
    const enumType = {
        kind: ENUM_TYPE_KIND,
        name: name,
        isFlags: false,
...
function parseComplexType(name, raw) {
    const complexType = { kind: COMPLEX_TYPE_KIND, name: name, properties: new Map() };
...
(namespace) => {
    ...
    const document = VOCABULARIES[namespace];
    if (!document) {
        return;
    }
 ...
 */
export const loadVocabulariesInformation = (includeCds, includeCdsAnalytics) => {
    // try to use cache
...
function propagateConstraintsForType(typeName, dictionary, derivedTypesPerType) {
    ...
    if (typeDef?.kind === COMPLEX_TYPE_KIND &&
        typeDef.constraints &&
...
function getFullyQualifiedAllowedTermName(allowedTerm) {
    ...
    const namespace = ALIAS_TO_NAMESPACE.get(segments[0]);
    if (!namespace) {
        return allowedTerm;
    }
