...
function toFullyQualifiedPath(namespaceMap, currentNamespace, path) {
    return path.segments
        ...
        .join(parse_js_1.PATH_SEPARATOR);
}
/**
    ...
    switch (segment.type) {
        case 'action-function': {
            const parameters = segment.parameters
                .map((parameter) => (0, index_js_1.toFullyQualifiedName)(namespaceMap, currentNamespace, parameter))
        ...
        case 'term-cast':
            return `@${(0, index_js_1.toFullyQualifiedName)(namespaceMap, currentNamespace, { ...segment, type: 'identifier' }) ?? ''}${segment.qualifier ? '#' + segment.qualifier : ''}`;
        case 'navigation-property-annotation':
            return `${(0, index_js_1.toFullyQualifiedName)(namespaceMap, currentNamespace, {
                type: 'identifier',
                name: segment.name,
                namespaceOrAlias: segment.namespaceOrAlias
            }) ?? ''}@${(0, index_js_1.toFullyQualifiedName)(namespaceMap, currentNamespace, { ...segment.term, type: 'identifier' }) ?? ''}${segment.term.qualifier ? '#' + segment.term.qualifier : ''}`;
        default:
            return '';
    }
}
