...
const ENTITY_TYPE_NAMES = new Set([Edm.EntityType, Edm.Singleton, Edm.EntitySet, Edm.NavigationProperty]);
...
const PARAMETER_TYPE_NAMES = new Set([Edm.Property, Edm.ReturnType, Edm.Parameter]);
 ...
 */
export function convertMetadataDocument(uri, document) {
 ...
 */
export function convertMetadataDocumentV2(uri, document) {
    const root = document.rootElement;
    if (!root) {
        return [[], []];
    }
...
function getOverloadName(context, element) {
    ...
    const name = getAttributeValue('Name', element);
    let parameterSubElements = element.subElements.filter((subElement) => subElement.name === Edm.Parameter);
...
function getKeys(element) {
    ...
    if (keyElements?.length) {
        keys = keyElements[0].subElements
            ...
            .map((propRefElement) => getAttributeValue('Name', propRefElement));
    }
    return keys;
}
/**
...
function getNamespaceMap(element) {
    ...
    const aliasMap = {};
    const includes = references.reduce((acc, reference) => [...acc, ...getElementsWithName('Include', reference)], []);
