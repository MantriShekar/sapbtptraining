import { Position, createTextNode, createElementNode, createAttributeNode, Edm, ELEMENT_TYPE } from '@sap-ux/odata-annotation-core';
 ...
 */
export function convertDocument(uri, ast) {
    if (ast.rootElement) {
        ...
        if (targetAttribute) {
            ...
            if (targetNamePosition) {
                ...
                const terms = getElementsWithName(Edm.Annotation, annotations)
                    .map(convertElement)
                    .filter((node) => node?.type === ELEMENT_TYPE);
...
function convertElement(element) {
    ...
    const attributes = element.attributes.reduce((acc, attribute) => {
        ...
        const attributeNode = createAttributeNode(attribute.key, value, transformRange(attribute.syntax.key), transformRange(attribute.syntax.value));
