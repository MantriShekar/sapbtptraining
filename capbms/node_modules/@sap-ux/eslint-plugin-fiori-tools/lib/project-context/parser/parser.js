...
export class ApplicationParser {
    ...
    parseService(projectRootPath, service) {
        if (service.type === 'cap') {
            const artifacts = CdsAnnotationProvider.getCdsServiceArtifacts(projectRootPath, service.path, this.context.fileCache);
            return artifacts ? [artifacts, []] : undefined;
        }
        else {
            return getXmlServiceArtifacts(service.version, service.path, { uri: service.metadata.uri, isReadOnly: true }, service.annotationFiles.map((file) => ({ uri: file.uri, isReadOnly: file.type === 'remote' })), this.context.fileCache);
        }
    }
    /**
