import { CONDENSED_TABLE_LAYOUT } from '../language/diagnostics.js';
...
const rule = createFioriRule({
    ...
    meta: {
        ...
        docs: {
            recommended: true,
            description: 'Requires condensedTableLayout to be enabled when using a grid table, analytical table, or tree table.',
        ...
        messages: {
            [CONDENSED_TABLE_LAYOUT]: '"condensedTableLayout" must be set to true when using a grid table, analytical table, or tree table.'
        },
        fixable: 'code'
    },
    check(context) {
 ...
 */
function checkCondensedTableLayoutForApp(app, parsedApp) {
    if (app.type === 'fe-v4') {
        return app.pages.flatMap((page) => page.type === 'list-report-page' ? checkCondensedTableLayoutV4(page, parsedApp) : []);
    }
    return app.pages.flatMap((page) => page.type === 'list-report-page' ? checkCondensedTableLayoutV2(page, parsedApp) : []);
}
/**
 ...
 * @returns - CondensedTableLayout issues
 */
function checkCondensedTableLayoutV4(page, parsedApp) {
    const problems = [];
 ...
 * @returns - CondensedTableLayout issues
 */
function checkCondensedTableLayoutV2(page, parsedApp) {
    const hasCompactTable = (page.lookup['table'] ?? []).some((table) => COMPACT_TABLE_TYPES.has(table.configuration.tableType.valueInFile ?? ''));
    if (!hasCompactTable) {
        return [];
    }
...
export default rule;
//# sourceMappingURL=sap-condensed-table-layout.js.map