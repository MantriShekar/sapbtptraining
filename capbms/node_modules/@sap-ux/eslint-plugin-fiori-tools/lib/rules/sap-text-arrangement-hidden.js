import { Edm, getElementAttributeValue, toFullyQualifiedName, parseIdentifier, ELEMENT_TYPE } from '@sap-ux/odata-annotation-core';
import { createFioriRule } from '../language/rule-factory.js';
import { TEXT_ARRANGEMENT_HIDDEN } from '../language/diagnostics.js';
...
import { resolveTextPropertyPath, collectRelevantEntityTypes, parseCommonTextAnnotationKey, getTextPath, getBoolValue } from './utils/common-text-helpers.js';
/**
...
function hasInlineTextArrangement(textElement, aliasInfo) {
    ...
    for (const child of textElement.content) {
        ...
        const qualifiedTerm = toFullyQualifiedName(aliasInfo.aliasMap, aliasInfo.currentFileNamespace, parseIdentifier(childTerm));
...
function processTextAnnotation(textAnnotation, entityTypeName, targetPath, pageNames, entityTypesWithTextArrangement, parsedService) {
    ...
    const hasPropertyLevelCommonTA = !!parsedService.index.annotations[buildAnnotationIndexKey(targetPath, COMMON_TEXT_ARRANGEMENT)];
    // UI.TextArrangement may also be applied directly on the entity type as a fallback
    const hasTextArrangement = hasInlineTA || hasPropertyLevelCommonTA || entityTypesWithTextArrangement.has(entityTypeName);
...
const rule = createFioriRule({
    ...
    meta: {
        ...
        messages: {
            [TEXT_ARRANGEMENT_HIDDEN]: 'The text property "{{textPropertyPath}}" referenced using the Common.Text annotation on "{{targetPath}}" is hidden (UI.Hidden). Remove the UI.Hidden annotation from the text property or set it to false.'
        }
    },
    check(context) {
        const problems = [];
