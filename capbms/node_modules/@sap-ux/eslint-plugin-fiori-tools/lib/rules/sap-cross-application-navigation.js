...
function isInterestingAssignment(node) {
    return (isGetServiceCall(node) ||
        (isLogical(node) &&
            (isInterestingAssignment(node.left) || isInterestingAssignment(node.right))));
}
/**
...
function isValid(node) {
    if (node.arguments?.length > 0) {
        ...
        if (target) {
            ...
            if ((shellHash && getName(shellHash) === '#Shell-home') ||
...
export default rule;
//# sourceMappingURL=sap-cross-application-navigation.js.map