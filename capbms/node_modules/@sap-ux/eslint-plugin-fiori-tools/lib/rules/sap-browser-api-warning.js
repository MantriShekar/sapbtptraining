 ...
 */
import { isType, isIdentifier, isMember, isCall, isLiteral, buildCalleePath, isForbiddenObviousApi } from '../utils/helpers.js';
// ------------------------------------------------------------------------------
// Helper Functions
// ------------------------------------------------------------------------------
/**
 ...
 */
function isDirectBodyAccess(calleePathNonCmpt, speciousObjectNonCmpt) {
    return ((calleePathNonCmpt === 'document.body' || calleePathNonCmpt === 'window.document.body') &&
...
const rule = {
    meta: {
        ...
        messages: {
            ...
            historyUsages: 'Direct history manipulation, does not work with deep links, use router and navigation events instead',
    ...
    create(context) {
        ...
        const FORBIDDEN_DOCUMENT_OBJECT = [], FORBIDDEN_SCREEN_OBJECT = [], FORBIDDEN_BODY_OBJECT = [], FORBIDDEN_HISTORY_OBJECT = [];
        const IF_CONDITION = 'IfStatement', CONDITION_EXP = 'ConditionalExpression', UNARY = 'UnaryExpression';
        // --------------------------------------------------------------------------
        // Helpers
        // --------------------------------------------------------------------------
        /**
         ...
         */
        function processMemberInit(node, init) {
         ...
         */
        function processIdentifierInit(node, init) {
            const varName = node.id.name;
         ...
         * @param node The call expression node
         */
        function handleHistoryForward(node) {
         ...
         * @param node The call expression node
         */
        function handleHistoryBack(node) {
         ...
         */
        function processHistoryMethod(node, methodName) {
            switch (methodName) {
                case 'forward':
         ...
         */
        function handleDomAccessChecks(node, methodName, speciousObject) {
         ...
         * @param speciousObject The object being accessed
         */
        function handleWindowSelectionChecks(node, methodName, speciousObject) {
         ...
         */
        function isBodyElementAccess(speciousObject, calleePath) {
            return ((speciousObject === 'body' && calleePath.includes('document.')) ||
                (speciousObject === 'body' &&
         ...
         * @param calleePath The full callee path
         */
        function handleBodyElementChecks(node, speciousObject, calleePath) {
         ...
         * @param methodName The method name
         */
        function handleForbiddenMethodCall(node, methodName) {
         ...
         * @param node The member expression node
         */
        function handleOtherMethodCall(node) {
            const calleePath = buildCalleePath(node);
            const speciousObject = isForbiddenObviousApi(calleePath);
         ...
         * @param node The member expression node
         */
        function handleCallExpressionMember(node) {
            if (!node.parent) {
                return;
            }
         ...
         */
        function isScreenAccess(calleePathNonCmpt) {
            return (calleePathNonCmpt === 'window.screen' ||
         ...
         */
        function isIndirectBodyAccess(calleePathNonCmpt, speciousObjectNonCmpt) {
            return ((speciousObjectNonCmpt === 'body' &&
                FORBIDDEN_DOCUMENT_OBJECT.includes(calleePathNonCmpt.slice(0, calleePathNonCmpt.lastIndexOf('.body')))) ||
         ...
         * @param node The member expression node
         */
        function handleNonCallExpressionMember(node) {
...
export default rule;
//# sourceMappingURL=sap-browser-api-warning.js.map