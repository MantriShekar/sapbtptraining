...
import { toFullyQualifiedName, parseIdentifier, getSingleTextNode, getElementAttribute, getElementAttributeValue } from '@sap-ux/odata-annotation-core';
 ...
 */
export function convertAnnotationFile(file, serviceName, options) {
    if (!file.references) {
        return [];
    }
    const annotations = [];
    const namespaceMap = getNamespaceMap(file.references);
    const namespace = file.uri.startsWith(GHOST_FILENAME_PREFIX) || !file.namespace ? serviceName : (file.namespace?.name ?? '');
...
function convertEmbeddedAnnotations(namespaceMap, currentNamespace, element, options) {
    return (element.content ?? [])
        .filter((child) => child.type === ELEMENT_TYPE && child.name === Edm.Annotation)
...
function convertExpression(namespaceMap, currentNamespace, element, options) {
    ...
    const children = (element.content ?? []).filter((child) => child.type === ELEMENT_TYPE && EXPRESSION_TYPES.has(child.name));
...
function convertRecord(namespaceMap, currentNamespace, recordElement, options) {
    ...
    const propertyValues = (recordElement.content || [])
        .filter((child) => child.type === ELEMENT_TYPE && child.name === Edm.PropertyValue)
        .map((propValueChild) => {
        if (options?.addOrigins) {
...
function convertCollection(namespaceMap, currentNamespace, collectionElement, options) {
    ...
    (collectionElement.content || [])
        .filter((child) => child.type === ELEMENT_TYPE)
