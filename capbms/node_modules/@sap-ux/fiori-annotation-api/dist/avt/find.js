 ...
 */
export function getAvtNodeFromPointer(root, pointer) {
 ...
 */
export function findAnnotation(annotationLists, reference) {
 ...
 */
export function findAnnotationByReference(aliasInfo, file, mergeMap, reference, pointer, mergeAnnotations, valueType) {
...
function searchInMergedAnnotations(file, mergeMap, id, pointer, valueType) {
    ...
    for (let segmentIndex = mergedSegments.length; segmentIndex > 0; segmentIndex--) {
        ...
        if (match) {
            ...
            const internalPointer = convertPointerInAnnotationToInternal(sourceTerm, '/' + [...mappedSourceSegments, ...mergedSegments.slice(segmentIndex)].join('/'), valueType);
            return {
                element: sourceTerm,
