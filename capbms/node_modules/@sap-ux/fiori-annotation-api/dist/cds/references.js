...
import { getElementAttributeValue, getPathBaseMetadataElement, getSegmentWithoutAlias } from '@sap-ux/odata-annotation-core';
    ...
    for (const annotation of target.terms) {
        ...
        if (annotation.range) {
            ...
            if (termName === 'Analytics.AggregatedProperties') {
                const collection = annotation.content.find((element) => element.type === ELEMENT_TYPE && element.name === Edm.Collection);
                if (!collection) {
                    continue;
                }
...
function collectVirtualProperties(virtualProperties, fileUri, metadataElement, collection) {
    for (const record of collection.content) {
        if (record.type === ELEMENT_TYPE && record.name === Edm.Record) {
            const nameElement = record.content.find((element) => element.type === ELEMENT_TYPE &&
                element.name === Edm.PropertyValue &&
                element.attributes[Edm.Property]?.value === 'Name');
            if (nameElement) {
                const valueElement = nameElement.content.find((element) => element.type === ELEMENT_TYPE && element.name === Edm.String);
                if (valueElement) {
                    const textNode = valueElement.content.find((element) => element.type === TEXT_TYPE);
...
function traverseCollection(definitionIndex, fileUri, prefix, collection) {
    const elements = collection.content.filter((node) => node.type === ELEMENT_TYPE);
 ...
 */
export const isAvailable = (projectRoot, references, relativePath, name) => {
 ...
 */
export function resolvePath(path, aliasInfo) {
...
function checkMetadataDefinitions(fileUri, aliasInfo, references, missingReferences, virtualProperties, entry, metadataService, projectRoot) {
    ...
    if (virtualPropertyUri &&
        !isAvailable(projectRoot, references, relative(projectRoot, fileURLToPath(virtualPropertyUri)), fullyQualifiedPath)) {
 ...
 */
export function getMissingRefs(documents, fileUri, targetName, targetOrElement, aliasInfo, metadataService, projectInfo) {
