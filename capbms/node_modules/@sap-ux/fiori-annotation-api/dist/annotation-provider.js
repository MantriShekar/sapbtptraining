...
import { processServices } from '@sap-ux/project-access';
import { VocabularyService } from '@sap-ux/odata-vocabularies';
import { createCdsCompilerFacadeForRootSync, createMetadataCollector, getMetadataElementsFromMap } from '@sap/ux-cds-compiler-facade';
 ...
 */
export function getXmlServiceArtifacts(odataVersion, path, metadataFile, annotationFiles, fileCache) {
    const adapter = new XMLAnnotationServiceAdapter({
        type: 'local-edmx',
        odataVersion: odataVersion,
        metadataFile,
        annotationFiles
    }, XML_VOCABULARY_SERVICE, { apps: {}, projectType: 'EDMXBackend', root: '' }, '');
...
export class CdsAnnotationProvider {
    ...
    static vocabularyService = new VocabularyService(true);
    /**
    ...
    static getCdsServiceArtifacts(rootPath, servicePath, fileCache) {
        ...
        for (const path of facade.getAllSourceUris()) {
            ...
            const annotationFile = toAnnotationFile(uri, this.vocabularyService, cdsAnnotationFile, metadataCollector, undefined, propagationMap, true).file;
...
function getAliasInfo(metadataService, vocabularyService, files) {
    ...
    for (const [uri, document] of Object.entries(files)) {
        const namespaces = getAllNamespacesAndReferences(document.namespace ?? { name: '', type: 'namespace' }, document.references);
        const aliasInfo = getAliasInformation(namespaces, metadataService.getNamespaces());
        const aliasInfoWithAllVocabularies = addAllVocabulariesToAliasInformation(aliasInfo, vocabularyService.getVocabularies());
