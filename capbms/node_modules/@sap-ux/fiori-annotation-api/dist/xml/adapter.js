...
import { convertDocument, convertMetadataDocument, convertMetadataDocumentV2, getNewAnnotationFile, serializeTarget } from '@sap-ux/xml-odata-annotation-converter';
import { ATTRIBUTE_TYPE, ELEMENT_TYPE, createAttributeNode, createElementNode, Edm, Edmx, createReference, TEXT_TYPE, Location } from '@sap-ux/odata-annotation-core-types';
import { getAliasInformation, getAllNamespacesAndReferences, getElementAttributeValue, isElementWithName, parseIdentifier } from '@sap-ux/odata-annotation-core';
import { MetadataService } from '@sap-ux/odata-entity-model';
import { INSERT_ELEMENT, UPDATE_ATTRIBUTE_VALUE, DELETE_ELEMENT, MOVE_ELEMENT, INSERT_ATTRIBUTE, REPLACE_ELEMENT, REPLACE_ATTRIBUTE, DELETE_ATTRIBUTE, INSERT_TARGET, REPLACE_TEXT } from '../types/index.js';
...
export class XMLAnnotationServiceAdapter {
    ...
    sync(fileCache) {
        ...
        if (this.collectV2Annotations) {
            [this.metadata, this.v2annotations] = convertMetadataDocumentV2(this.service.metadataFile.uri, metadataDocument);
        }
        else {
    ...
    removeReferences(writer, document, aliasInfo, usedNames, pointer) {
        ...
        for (const namespace of toRemove) {
            for (let index = 0; index < document.ast.rootElement.subElements.length; index++) {
                ...
                const match = reference.subElements.find((child) => child.name === Edm.Include &&
...
function getSchemaPointer(document) {
    ...
    const dataServicesIndex = document.rootElement.subElements.findIndex((element) => element.name === Edmx.DataServices);
    if (dataServicesIndex === -1) {
        return undefined;
    }
    const schemaIndex = document.rootElement.subElements[dataServicesIndex].subElements.findIndex((element) => element.name === Edm.Schema);
    if (schemaIndex === -1) {
        return undefined;
    }
